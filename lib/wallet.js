...
var keyHash = bitcoinjs.crypto.hash160
...
var Wallet = module.exports = function (params, store, chain, opts, cb) {
...
Wallet.prototype.send = function (recipient, amount, cb) {
  ...
  async.eachSeries(inputs, function (input, cb) {
    ...
    self.store.get(k, function (err, keyInfo) {
      ...
      var key = self.rootKey.derivePath(keyInfo.path)
      tx.sign(key.privateKey, Transaction.SIGHASH_ALL)
      cb(null)
    })
  }, function (err) {
    if (err) return cb(err)
...
Wallet.prototype.getUTXO = function (amount, exclude) {
  ...
  for (var id in this.unspent) {
    ...
    var diff = Math.abs(output.value - amount)
  ...
  var script = this.unspent[closest].script
  return {
    id: closest,
    address: address.fromOutputScript(script, this.params),
    txid: new Buffer(closest.split(':')[0], 'base64').toString('hex'),
    outputIndex: +closest.split(':')[1],
    script: script,
    value: this.unspent[closest].value
  }
}
...
Wallet.prototype.isOwnOutput = function (output, cb) {
  ...
  script.isPubKeyOutput(output.script)) {
    var addr = address.fromOutputScript(output.script, this.params)
...
Wallet.prototype.isOwnInput = function (input, cb) {
  ...
  script.isPubKeyInput(input.script)) {
...
Wallet.prototype.total = function (items) {
  var amount = 0
  items.forEach(function (item) { amount += item.value })
  return amount
}
...
Wallet.prototype._processOutputs = function (tx, cb) {
  ...
  this.getOwnOutputs(tx.transaction, function (err, outputs) {
    ...
    var addresses = outputs.map(function (output) {
      // TODO: handle outputs with no address
      return address.fromOutputScript(output.script, self.params)
    })
...
Wallet.prototype._loadRootKey = function (cb) {
  ...
  this.store.get('root', opts, function (err, root) {
    if (err && !err.notFound) return cb(err)
    if (err && err.notFound) {
      crypto.randomBytes(SEED_SIZE, function (err, seed) {
        if (err) return cb(err)
        self.rootKey = HDNode.fromSeedBuffer(seed)
        var serialized = self.rootKey.toBase58()
        self.store.put('root', serialized, opts, function (err) { cb(err, self.rootKey) })
      })
      return
    }
    self.rootKey = new HDNode(root)
...
Wallet.prototype._loadState = function (cb) {
  ...
  this.store.get('state', function (err, state) {
    ...
    if (state.tip) {
      ...
      state.tip.header = Block.fromBuffer(header)
    }
    self.state = state
    cb(null, self.state)
  })
